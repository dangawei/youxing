            ...
            <div style="text-align:center">
                <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="96">
                    <FormItem label="账号:" prop="username">
                        <Input v-model="formValidate.username" placeholder="请输入11位手机号" onkeydown="if(event.keyCode==32) return false"></Input>
                    </FormItem>
                    <FormItem label="密码:" prop="password">
                        <div class="" v-if="passwordshow">
                           <Input type="password" v-model="formValidate.password" placeholder="请输入密码" icon="eye-disabled"  @on-click="password01()" onkeydown="if(event.keyCode==32) return false"></Input>
                        </div>
                        <div class="" v-else>
                           <Input v-model="formValidate.password" placeholder="请输入密码" icon="eye" @on-click="password01()" onkeydown="if(event.keyCode==32) return false"></Input>
                        </div>
                    </FormItem>
                    ...
                    <FormItem label="角色:" prop="roleId">
                        <get-account-role @exportData="exportData"></get-account-role>
                    </FormItem>
                    <FormItem>
...
<script>
    import getAccountRole from "@/components/pub/getAccountRole"
    export default {
        props:['addshow'],
        components:{
            getAccountRole
        ...
        created(){
            this.formValidate.companyId=this.$webapi.get("companyId");
        },
        mounted() {
